...
CRYPTOKI.smartStickyHeader = function () {
    ...
    function stickyHeader() {
        ...
        if (window.matchMedia("(max-width: 800px)").matches && !nav.classList.contains('nav-ds') && !nav.classList.contains('sticky-header')) {
        ...
        if (sticky !== null) {
            if (scrollY > sticky.offsetHeight) {
                if (scrollingDirection < 0) {
                    ...
                    sticky.setAttribute('style', `top: -${sticky.offsetHeight}px`)
                }
            } else {
